...
import {state, getRelativePath, resolveConfigFor, addIdToIdCache, getGlobalCssFiles, findIdFromCache, getFreeId, generateCssModuleHash, insertLinkIntoHead, findHtmlDeps, getNumberSuffix, replaceLast, removeIdFromCache} from '../shared.js';
...
async function writeCssAndHtml(cssFiles, htmlDoms, asts) {
    ...
    const {runtimeMap, config, scopeHashFileMap, scopeHashsMap, allCombis, mergeCssMap, outputCss} = state;
    ...
    const cssFilesObjs = (await Promise.all (cssFiles.map (async file => { 
       const obj= {
    ...
    const isGlobal = globalCssFiles.includes (file);
      ...
      if (isGlobal)
      {
        ...
        if (content.includes ('.'))
          console.warn('Classes detected in global CSS. Consider scoping these instead.');
      ...
      if (cachedId && freeId > cachedId.id)
        scopeIndex = cachedId.id;
      else 
      {
        scopeIndex = freeId;
      }
     ...
     //   scopeHashsMap[fileName] = new Set();
      ...
      const rulesArr = [];
          ...
          async (root) => {
            ...
            if (hash === 'READ') 
            {
                ...
                if (resolveTag)
                {
                  ...
                  if (next && next.type === 'comment' && next.source.start.line === hashDecl.source.end.line) {
                      next.remove();
                      }
            ...
            scopeHashFileMap[file] = hash;
              ...
              if (suffixOverride !== false)
                hashApply = suffixOverride;
              ...
              if (resolveTag)
                console.log (`Resolved to ${selectorsObj.hashedName}`);
            ...
            root.walkRules((rule) => {
              ...
              {
                if (rulesArr.includes (rule.parent))
                  return;
        ...
        if (hashRead) {
          ...
          if (mapObj) {
            ...
            const root = cssRoot;
            fileFound = { filePath: path.join(state.config.outputDir, filePath), cssRoot: root };
            ...
            root.walkRules(rule => {
              for (const decl of rule.nodes || []) {
                if (decl.prop === '--scope-hash' && decl.value.split(' ')[0].trim() === hashRead) {
              ...
              if (rule.type === 'rule')
                {
                  if (rule.selector?.startsWith (targetClass))
                    ...
                    break;
                } else if (rule.parent?.type === 'atrule' && rule.parent.name === 'media' )
                  rulesToRemove.push (rule);
            }
        
        
            root.walkAtRules('keyframes', atRule => {
              if (atRule.params.startsWith(`${targetClass.replaceAll('.', '')}__`)) {
      ...
      if (fileFound)
        ...
        if (delayedWrite)
          await fs.promises.writeFile (file, delayedWrite, 'utf-8');
    }
    ...
    function processNodes(nodes, currentScope = {}, inForm = false, ignoreScope = false) {
      nodes.forEach((node) => {
        ...
        if (node.attribs?.hasOwnProperty('data-break')) {
          ignoreScopeData = node.attribs['data-break'] ? node.attribs['data-break'].split (',') : '';
        ...
        {
          excludeData = node.attribs['data-exclude'] ? node.attribs['data-exclude'].split (',') : '';
        ...
        if (scope.hasOwnProperty('name')) {
          ...
          if (inForm)
          {
            let formProp = node.attribs?.for ? 'htmlFor' : node.attribs?.id ? 'id' : '';
            if (formProp)
            {
              if (!formProp.includes ('__'))
                node.attribs[formProp] = `${scope.hashedName}__${node.attribs.htmlFor}`;
              else 
                node.attribs[formProp] = node.attribs.htmlFor.replace (`${scope.name}__`, `${scope.hashedName}__`);
            }
          }
          ...
          if (node.attribs?.class) {
            ...
            for (const cls of classes) {
              ...
              else if (exclude && (!excludeData || excludeData.length <= 0 || excludeData.includes (cls)))
              {
                const flatIndex = flatClasses.findIndex (flatCls => flatCls.endsWith (`__${cls}`));
                if (flatIndex !== -1)
                  flatClasses[flatIndex] = cls;
                else
                  flatClasses.push (cls);
              }
              else if (ignoreScope && (!ignoreScopeData || ignoreScopeData.length <= 0 || ignoreScopeData.includes (cls)))
              {
                const flatIndex = flatClasses.findIndex (flatCls => flatCls.endsWith (`__${cls}`));
                if (flatIndex !== -1)
                  flatClasses[flatIndex] = cls;
            ...
            if (scope.config.dontFlatten) {
              ...
              {
                flatClasses.push(
                  ...classes.filter (cls => !flatClasses.includes(cls)).map((cls) =>
              ...
              {
                flatClasses.push (...classes.filter (cls => cls !== scope.name).map (cls => prevScope.name ? cls.replace (`${prevScope.name}__`, `${prevScope.hashedName}__`) : cls))
              }
            } /*else {
        ...
        if (node.children) processNodes(node.children, scope || currentScope, inForm, ignoreScope);
      });
    }
          ...
          if (tag.relativePath === filePath) {
            ...
            const foundPath = filesFound[filePath];
                if (!state.config.mergeCss)
                  insertLinkIntoHead(
                    dom,
                    foundPath ? getRelativePathForLink (foundPath, outPath) : tag.content.replace(`${state.config.inputDir}/`, `${state.config.outputDir}/`)
                  );
                ...
                for (const { raw, flat } of valueObj.selectors) {
                  ...
                  matches.forEach((match) => {
                    ...
                    if (typeof flat === 'object') processPseudoNode(match);
                      ...
                      if (!match.attribs.flatClasses)
                        match.attribs.flatClasses = [];
                      match.attribs.flatClasses.push(getAfterDot (removeFirstDot (flat)));
                    }
                  });
                }
              }
  ...
  */
    ...
    if (asts?.length > 0)
    {
        for (const ast of asts)
        {
            ...
            const out = ast.filePath.endsWith ('.jsx') ? await state.jsFormatter (raw) : await state.tsFormatter (raw);
    ...
    if (runtimeMapKeys.length > 0) {
      ...
      const outPath = state.config.writeRuntimeMap === true ? 'auto-scope-runtime-map.json' : state.config.writeRuntimeMap;
  ...
  function splitSelectorIntoSegments(sel) {
    ...
    function splitSelectors(selectorString) {
      ...
      function removeLinkFromHead(dom, href) {
        ...
        const node = DomUtils.findOne (el => el.name === 'link' && el.attribs.href === href, dom.children, true);
...
async function readGlobalCss ()
{
  ...
  const cssFiles = (await globby (state.config.inputCss)).filter (file => !globalCssFiles.includes (file));
  ...
  for(const [scopeName, val] of Object.entries (state.globalCssCache))
  {
    if (state.scopeHashsMap[scopeName])
      state.scopeHashsMap[scopeName] = new Set (Array.from (state.scopeHashsMap[scopeName]).filter (h => !val.hashes.includes (h)));
    for (const number of val.numbers)
      removeIdFromCache (scopeName, number);
  ...
  for (const file of globalCssFiles) {
      ...
      (root) => {
          root.walkRules((rule) => {
            ...
            selectors.forEach (selector =>
            {
              ...
              if (id)
                scopeName = replaceLast (selector, `-${id}`, '');
              else 
                scopeName = selector;
    ...
    const cssDeps = cssFiles.filter (file => scopesCovered.has (path.basename (file, '.css')));
  
    await writeCssAndHtml (cssDeps, findHtmlDeps (cssDeps), findHtmlDeps (cssDeps, true));
